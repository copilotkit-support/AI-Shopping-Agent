...
class ShoppingAgentFlow(Flow[AgentState]):
    ...
    async def start(self):
        try:
            ...
            if self.state.messages[-1]['role'] == 'ai':
            ...
            if(self.state.messages[-1]['role'] == 'tool'):
                ...
                # response = await model.ainvoke(input=self.state.messages)
                # self.state.messages.append(AIMessage(content=response.content, type="ai", id= self.state.messages[-2].tool_calls[0]['id']))
                response = completion(
            ...
            if hasattr(response0.choices[0].message, "tool_calls") and response0.choices[0].message.tool_calls and response0.choices[0].message.content == '':        
                ...
                # self.state.messages.append(AIMessage(id=str(uuid.uuid4()), type="ai",  tool_calls=response0.tool_calls, content=''))
                return self.state
                return Command(
                    goto=END,
                    update={
            ...
            if (not response0.choices[0].message.content.startswith('SEARCH')):
                ...
                # self.state.messages.append(AIMessage(id=str(uuid.uuid4()), type="ai",  content=response0.content))
                self.state.logs = []
                await copilotkit_emit_state(self.state)
                return self.state
                return Command(
            ...
            self.state.messages.append({
                "role" : "assistant",
                "content" : "",
                "tool_calls" : [{"name": "list_products", "args": {"products": self.state.buffer_products[:5], "buffer_products" : self.state.buffer_products, "chat_name" : chat_name}, "id": str(uuid.uuid4())}]
            })
            self.state.logs = []
            await copilotkit_emit_state(self.state)
            self.state.show_results = True
